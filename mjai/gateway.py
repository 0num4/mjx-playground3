...
class OpenCodeGen:
    ...
    def from_mjai_kakan(ev: dict[str, Any], obs: dict[str, Any]) -> tuple[int, list[int], int, int]:
        ...
        # pon flag 繧貞炎髯､縺励※ kan flag 繧堤ｫ九※繧
    ...
    def from_mjai_pon(ev: dict[str, Any], obs: dict[str, Any]) -> tuple[int, list[int]]:
        ...
        if ev["actor"] == obs["who"]:
            ...
            not_pon = available_codes[0]  # pon 縺励※縺縺ｪ縺迚
        else:
    ...
    def from_mjai_chi(ev: dict[str, Any], obs: dict[str, Any]):
        ...
        if ev["actor"] == obs["who"]:
            ...
            if consumed[0].endswith('r'):
                ...
                if consumed[0].startswith('5'):
                    consume0_candidates.remove(consume0_base)  # 襍､繝峨Λ繧帝勁螟悶☆繧
            ...
            if consumed[1].endswith('r'):
                ...
                if consumed[1].startswith('5'):
                    consume1_candidates.remove(consume1_base)  # 襍､繝峨Λ繧帝勁螟悶☆繧
        ...
        # pai, consumed 縺ｫ襍､繝峨Λ縺後≠繧句 ｴ蜷医ｮ縺溘ａ縺ｫ ignore_aka=True 縺ｨ豈碑ｼ縺吶ｋ
        if called == 0:
...
class MjxGateway:
    ...
    def _get_mjx_obs(self, mjai_events):
        # 1. MJAI 縺ｮ蜈･蜉帙ｒ MJX 縺ｫ螟画鋤縺励※ Game Client 縺ｫ貂｡縺
            ...
            match mjai_event_type:
                ...
                case "dahai":
                    ...
                    if self.actor_id == mjai_event["actor"]:
                        ...
                        # 閾ｪ蛻縺ｧ縺ｯ縺ｪ縺蝣ｴ蜷医∵眠隕上↓蜃ｺ迴ｾ縺吶ｋ縺溘ａ hai 繧偵う繝ｳ繧ｯ繝ｪ繝｡繝ｳ繝医☆繧
                        hai_ = hai + self.hai_offset.get(hai, 0)
                        self.hai_offset[hai] = self.hai_offset.get(hai, 0) + 1
                ...
                case _:
                    # self.base_obs 縺ｫ螟画峩繧定｡後ｏ縺ｪ縺
                    continue

        # legal action 繧剃ｻ倅ｸ弱☆繧
        ...
        # legal action 繧剃ｻ倅ｸ弱＠縺滉ｸ翫〒 act 繧貞他縺ｶ
    ...
    def _get_mjai_response(self, mjx_action):
        """
        MJX 縺ｮ Action 繧 MJAI 縺ｫ螟画鋤縺吶ｋ

        ACTION_TYPE_* 縺ｮ縺吶∋縺ｦ縺ｮ繝代ち繝ｼ繝ｳ縺ｫ蟇ｾ縺吶ｋ謖吝虚繧貞ｮ夂ｾｩ縺吶ｋ
        ...
        - [X] ABORTIVE_DRAW_NINE_TERMINALS = mjxproto.ACTION_TYPE_ABORTIVE_DRAW_NINE_TERMINALS  # 荵晉ｨｮ荵晉煙
    ...
    def react(self, events_str: str) -> str:
        ...
        # 遨ｺ縺ｧ縺ｯ縺ｪ縺繝ｪ繧ｹ繝医′荳弱∴繧峨ｌ繧
        assert len(events) > 0

        # 譛蠕後ｮ繧､繝吶Φ繝医ｮ `type` 縺ｫ繧医▲縺ｦ蛻蟯舌☆繧
        if events[-1]["type"] in ["start_game", "end_kyoku", "end_game"]:
            return json_dumps({"type": "none"})
        else:
            # 1. MJAI 縺ｮ蜈･蜉帙ｒ MJX 縺ｫ螟画鋤縺励※ Game Client 縺ｫ貂｡縺
            # 2. MJX 縺ｮ Action 繧 MJAI 縺ｫ螟画鋤縺吶ｋ
